...
if ($result->num_rows > 0){
    ...
    // 更新密码和状态
    $stmt = $conn->prepare("UPDATE user SET Password = ?, status = 0, verification_code = ? WHERE Email = ?");
    $stmt->bind_param("sss", $Password, $verification_code, $Email);
    $stmt->execute();
    ...
    try {
        ...
        $mail->Subject = 'Activate your New Password now ' . time();
        $mail->Body = 
        '<!DOCTYPE html>
        <html lang="en">
        <head>
    ...
    echo '<script>
        alert("no such user");
        window.location.href="Reset Page.php";
    </script>';
    exit; // 确保后续代码不会被执行
}
